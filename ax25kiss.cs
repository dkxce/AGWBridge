...
namespace ax25kiss
{
    public class Packet
    {
        public  const byte KISS_FEND = 0xc0;
        ...
        {
            if (digipath == null) digipath = new string[0];
        ...
        // packet from Kiss Full Frame
        public static Packet FromKissFullFrame(byte[] incomingPacket)
        {
            if (incomingPacket == null) return null;
            if (incomingPacket.Length < 3) return null;
            if (incomingPacket[0] != KISS_FEND) return null;
            if (incomingPacket[incomingPacket.Length - 1] != KISS_FEND) return null;
            if (incomingPacket[1] != KISS_CMD_DATAFRAME0) return null;
            byte[] res = new byte[incomingPacket.Length - 3];
            Array.Copy(incomingPacket, 2, res, 0, res.Length);
            return new Packet(res);
        }

        public static byte[] KissDataFrame(string destination, string source, string[] digipath, int control, int protocol, byte[] frame)
        {
            if (digipath == null) digipath = new string[0];
            ...
            Array.Copy(packet, 0, res, 0, size);
            return res;
        }

        public static byte[] KissFullFrame(string destination, string source, string[] digipath, string data)
        {
            return ax25kiss.Packet.KissDataFrame(destination, source, digipath, ax25kiss.Packet.AX25_CONTROL_APRS, ax25kiss.Packet.AX25_PROTOCOL_NO_LAYER_3, System.Text.Encoding.ASCII.GetBytes(data));
        }

        public static byte[] KissFullFrame(string destination, string source, string[] digipath, byte[] data)
        {
            return ax25kiss.Packet.KissDataFrame(destination, source, digipath, ax25kiss.Packet.AX25_CONTROL_APRS, ax25kiss.Packet.AX25_PROTOCOL_NO_LAYER_3, data);
        }

        public static byte[] KissFullFrame(byte[] kissDataFrame)
        {
            List<byte> kissFrame = new List<byte>();
            kissFrame.Add(KISS_FEND);
            kissFrame.Add(KISS_CMD_DATAFRAME0);
            kissFrame.AddRange(kissDataFrame);
            kissFrame.Add(KISS_FEND);
        ...
        private static void addCallsign(byte[] bytes, int offset, string callsign, bool last)
        {
            int i;
            bool call_ended = false;
            char c = ' ';
